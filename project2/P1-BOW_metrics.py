...
y_train_df = np.array(y_train_df['is_positive_sentiment'].values.tolist())
...
for i in range(0,len(y_predict_on_train)):
    if y_predict_on_train[i] < y_train_df[i]:
        print("Found FN: %s" %tr_text_list[i]) 
        FN_list.append(tr_text_list[i])
    if y_predict_on_train[i] > y_train_df[i]:
        print("Found FP: %s" %tr_text_list[i])  
        FP_list.append(tr_text_list[i])
...
x_test_NF = X.toarray()
yproba1_te_N = gnb_cv.predict_proba(x_test_NF)[:,1]
np.savetxt("yproba1_test.txt", yproba1_te_N, delimiter=",")
