...
def bootstrap_sample(x_NF,y_NF, random_state = np.random):
    ...
    return x_NF[row_ids].copy(), y_NF[row_ids].copy(),row_ids
...
for i in range(0,range1):
    x_i, y_i,num = bootstrap_sample(X_NF,y_NF)
    x_list.append(x_i)
    y_list.append(y_i)
    identity.append(num)
...
# gnb_params = {'C':[100], 'solver':['newton-cg']}
# gnb_params = {'C':[0.0001,0.01,1,100,10000]} LR
gnb_params = {'var_smoothing':[0, 0.1, 1, 10, 100]}
# gnb_params = {'n_neighbors':[1,3,5,7,10]}


y_predict_on_train = 0
for x_NF,y_NF,indexes in zip(x_list,y_list,identity):
    run = run +1
...
np.savetxt("P2_SVC_FP.txt", FP_list, delimiter=",",fmt="%s")
np.savetxt("P2_SVC_FN.txt", FN_list, delimiter=",",fmt="%s")
