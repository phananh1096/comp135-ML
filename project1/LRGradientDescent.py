...
class LogisticRegressionGradientDescent():
    ...
    * the extra L2 penalty term will only ever add to the loss
    ...
    def predict_proba(self, x_NF):
        ...
        # Hint: Be sure to use a numerically stable logistic_d function

        x_bias_NG = self.insert_final_col_of_all_ones(x_NF)
        z = np.dot(self.w_G[np.newaxis,:], np.transpose(x_bias_NG))[0]
        yproba0_N1 = np.where(z >= 0, 1-sigmoid_pos(z), 1-sigmoid_neg(z))
        yproba1_N1 = np.where(z >= 0, sigmoid_pos(z), sigmoid_neg(z))
        yproba0 = np.array(yproba0_N1).reshape(N,1)
#         print(np.array(yproba0_N1).reshape(N,1))
#         print(yproba0.shape)
        yproba1 = np.array(yproba1_N1).reshape(N,1)
#         print(yproba1)
#         print(yproba1.shape)
        yproba_N2 = np.hstack([yproba0, yproba1])
#         print(yproba_N2)
        return yproba_N2
        ...
        * trace_L1_norm_of_grad contains L1 norm of grad after every step
        ...
        for iter_id in range(self.num_iterations + 1):
            if iter_id > 0:
                ...
                w_G = w_G - self.step_size*self.calc_grad(w_G,xbias_NG,y_N) # <- TODO replace this line
    ...
    def calc_loss(self, w_G, xbias_NG, y_N):
        ...
        N = float(y_N.size)
        denom = N * np.log(2)
        z = np.dot(w_G[np.newaxis,:], np.transpose(xbias_NG))[0]
#          print(z)
        
        ## First term: Calc loss due to L2 penalty on weights
        L2_loss = (self.alpha * np.dot(w_G[:np.newaxis],w_G))/2
    
        ## Second term: Calc log loss by summing over examples
        yz = np.dot(y_N, z)
        ls = logsumexp(np.hstack([z.reshape(int(N),1), np.zeros((int(N),1))]),axis=1)
        log_loss = yz-np.sum(ls) # TODO calc log loss in numerically stable way
    ...
    def calc_grad(self, w_G, xbias_NG, y_N):
        ...
        grad_logloss_wrt_w_G = []
        z = np.dot(w_G[np.newaxis,:], np.transpose(xbias_NG))[0]
        z_sig = []
        #Test z_sig
        z_sig = np.where(z >= 0, sigmoid_pos(z), sigmoid_neg(z))
        y_minz = y_N - z_sig
        for g in range(G):
            grad_logloss_wrt_w_G.append(np.dot(y_minz,xbias_NG[:,g]))
